...
@Composable
internal fun FavoriteVacanciesScreen(
    onBack: () -> Unit,
    onVacancyClick: (Long) -> Unit,
) {
    Screen<FavoriteVacanciesViewModel> { viewModel ->
        ...
        viewModel.sideEffect.collectAsEffect { effect ->
            when (effect) {
                FavoriteVacanciesSideEffect.NavigateBack -> onBack()
                is FavoriteVacanciesSideEffect.NavigateToVacancyDetails -> onVacancyClick(effect.vacancyId)
            }
        }
    }
}
...
private fun Content(
    ...
    onAction: (FavoriteVacanciesAction) -> Unit,
) {
    FullscreenColumn(modifier = Modifier.fillMaxSize()) {
        TopBar(onBack = { onAction(FavoriteVacanciesAction.NavigateBack) })
        VerticalSpacer(NotDjinniTheme.offsets.medium)
        when (val contentState = state.contentState) {
            FavoriteVacanciesContentState.Loading -> LoadingContent()
            FavoriteVacanciesContentState.Empty -> EmptyContent()
            is FavoriteVacanciesContentState.Error -> ErrorContent(
                message = contentState.message,
                onRetry = { onAction(FavoriteVacanciesAction.Load) }
            )
            is FavoriteVacanciesContentState.Data -> DataContent(
                vacancies = contentState.vacancies,
                onVacancyClick = { vacancyId ->
                    onAction(FavoriteVacanciesAction.OpenVacancy(vacancyId))
                },
                onFavoriteClick = { vacancyId ->
                    onAction(FavoriteVacanciesAction.RemoveFavorite(vacancyId))
                }
            )
        }
    }
...
private fun TopBar(onBack: () -> Unit) {
    Row(
        ...
        NotDjinniText(
            data = R.string.favorite_vacancies_title.toTextData(),
...
private fun EmptyContent() {
    Box(
        ...
        NotDjinniText(
            data = R.string.favorite_vacancies_empty.toTextData(),
    ...
    LazyColumn(
        ...
        items(
            ...
            VacancyCard(
                ...
                onFavoriteClick = { onFavoriteClick(vacancy.id) }
            )
        }
    }
}
...
private fun Preview() {
    NotDjinniTheme {
        val state = FavoriteVacanciesState(
            contentState = FavoriteVacanciesContentState.Empty
        )
