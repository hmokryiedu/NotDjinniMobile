...
@Composable
fun EmploymentTypeSelectionAlert(
    types: List<EmploymentType>,
    selected: EmploymentType?,
    onDismiss: () -> Unit,
    onApply: (EmploymentType) -> Unit
) {
    ...
    Dialog(
        ...
        Column(
            ...
            NotDjinniText(
                data = R.string.employment_type_dialog_title.toTextData(),
            ...
            LazyColumn {
                items(items = types, key = { it.name }) { type ->
                    SelectionItem(
                        type = type,
                        isSelected = type == selectedEmploymentType,
                        onSelect = { selectedEmploymentType = type }
                    )
                }
            }
            VerticalSpacer(NotDjinniTheme.offsets.medium)
                ...
                NotDjinniText(
                    modifier = Modifier
                        .clickableNoRippleWithoutInterval {
                            onDismiss()
                            onApply(selectedEmploymentType ?: return@clickableNoRippleWithoutInterval)
                        }
                        .padding(NotDjinniTheme.offsets.small),
                    data = R.string.apply.toTextData(),
                    style = NotDjinniTheme.typography.body2,
                    color = if (selectedEmploymentType != null) {
